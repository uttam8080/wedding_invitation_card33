...
class Media {
  ...
  update(scroll, direction) {
    ...
    if (this.bend === 0) {
      ...
      const bendFactor = isMobile ? Math.abs(this.bend) * 1.5 : Math.abs(this.bend);
      const B_abs = bendFactor;
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const widthFactor = isMobile ? 320 : 650;
    const heightFactor = isMobile ? 480 : 850;
...
export function CircularGallery({
  ...
  textColor = '#8B1E3F',
  borderRadius = 0.08,
  font = 'bold 24px Cormorant Garamond',
  scrollSpeed = 2,
  scrollEase = 0.05,
  className = ""
}) {
